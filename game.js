(() => {
  ...
  const ui = {
    loading: $('#loading-screen'), briefing: $('#briefing-screen'), result: $('#result-screen'),
    loadingProgress: $('#loading-progress'), loadingCopy: $('#loading-copy'),
    start: $('#start-button'), restart: $('#restart-button'), continue: $('#continue-button'),
  ...
  class DebrisGame {
    ...
    updateSpawning(dt) {
      ...
      const base = lerp(1.3, .78, progress);
  ...
  function setupUi() {
    ...
    ui.continue.addEventListener('click', () => {
      ...
      ui.continue.textContent = 'RESULT TRANSMITTED';
      ui.continue.disabled = true;
      setTimeout(() => { ui.continue.textContent = 'CONTINUE'; ui.continue.disabled = false; }, 1600);
    });
  }
  ...
  async function initialise() {
    ...
    try {
      ...
      if (query.get('autostart') === '1') setTimeout(() => game.start(), 300);
    } catch (error) {
      console.error(error);
